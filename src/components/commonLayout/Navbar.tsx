        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="hidden lg:flex gap-8">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-4 w-20 bg-gray-100/50 rounded animate-pulse"
              />
            ))}
          </div>
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {menuItems.map((item, idx) => {
              const itemType =
                item.type ||
                (item.items && item.items.length > 0 ? "dropdown" : "link");
              return (
                <div
                  key={idx}
                  className="relative"
                  onMouseEnter={() =>
                    itemType === "dropdown" && setActiveDropdown(idx)
                  }
                ...
                >
                  {itemType === "link" ? (
                    <Link
                      href={item.href || "#"}
                      className="px-5 py-2.5 rounded-full text-[14px] font-medium text-gray-600 hover:text-pink-600 transition-all duration-300 hover:bg-pink-50/50"
                    >
                      {language === "en" ? item.titleEn : item.titleBn}
                    </Link>
                  ) : (
                    <button
                      className={`px-5 py-2.5 rounded-full text-[14px] font-medium flex items-center gap-1.5 transition-all duration-500 ${
                        activeDropdown === idx
                    ...
                    >
                      ...
                      <ChevronDown
                        size={14}
                        className={`transition-transform duration-500 ${activeDropdown === idx ? "rotate-180" : ""}`}
                      />
                    </button>
                  )}
                      ...
                      >
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-1 mb-1">
                                  <span className="text-sm font-semibold text-gray-900 group-hover/item:text-pink-600 transition-colors">
                                    {language === "en"
                                      ? sub.nameEn
                                      : sub.nameBn}
                                  </span>
                                  <ChevronRight
                                    size={14}
                                    className="opacity-0 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all text-pink-600"
                                  />
                                </div>
                        ...
                        <div className="bg-gray-50/80 p-5 px-8 flex justify-between items-center border-t border-gray-100">
                          <p className="text-[12px] text-gray-500 font-medium">
                            {language === "en"
                              ? "Explore our comprehensive suite of messaging tools."
                              : "আমাদের সব মেসেজিং টুলসগুলো দেখুন।"}
                          </p>
                          <Link
                            href="/contact"
                            className="group/link text-[12px] font-black text-pink-600 flex items-center gap-1 hover:gap-2 transition-all"
                          >
                            {language === "en"
                              ? "Contact Sales"
                              : "বিক্রয়ের জন্য যোগাযোগ করুন"}
                            <ArrowRight size={14} />
                          </Link>
                        </div>
                      </motion.div>
          ...
          <div className="flex items-center gap-2 group relative z-50">
            ...
            >
              <Globe
                size={16}
                className="text-gray-400 group-hover/lang:text-pink-500 transition-colors"
              />
            ...
            <Link
              href="/login"
              className="hidden sm:block px-5 py-2 rounded-full text-[14px] font-medium text-gray-600 hover:text-pink-600 transition-all"
            >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4 pt-4 border-t border-gray-50">
                {[
                  ...
                  >
                    <span className="text-lg font-semibold text-gray-700 group-hover:text-pink-600">
              ...
              <div className="pt-4 space-y-4">
                <Link
                  href="/login"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="block w-full text-center py-4 rounded-2xl bg-gray-50 text-gray-900 font-semibold text-sm"
                >
                  {language === "en" ? "Sign In" : "লগইন"}
                </Link>
              </div>
