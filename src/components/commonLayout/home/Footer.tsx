...
const Footer = () => {
  ...
  const companyName =
    language === "en" ? config.company.nameEn : config.company.nameBn;
  const tagline =
    language === "en" ? config.company.taglineEn : config.company.taglineBn;
  const copyright =
    language === "en" ? config.company.copyrightEn : config.company.copyrightBn;
  ...
  return (
    <footer className="relative w-full bg-[#0a0a0c] text-white pt-24 pb-12 overflow-hidden">
      ...
      <div
        className={`relative ${themeConfig.spacing.container.maxWidth} mx-auto ${themeConfig.spacing.container.padding}`}
      >
        {/* Main Footer Content */}
